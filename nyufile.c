...
int main(int argc, char *argv[])
{
    ...
    while ((ch = getopt(argc, argv, "ilr:s:R:")) != -1)
    {
        ...
        case 'l':
            ...
            break;
        case 's':
            // parse hex string into a hex num; 2 hexdecimal digit ==>  1 byte
            for (int j = 0; j < SHA_DIGEST_LENGTH * 2; j += 2)
            {
                // reference for using sscanf to convert hex string:
                // https://www.ibm.com/docs/en/zos/2.3.0?topic=programs-sscanf-read-format-data
                // how this works: treat bytes starting from optarg + j as an integer (in hexadecimal format),
                // keep only the first two (cuz %2), store that into sha1 + j / 2
                // optarg increments by two, sha1 increments by 1, repeat
                // the result is: every two "chars" in optarg gets stored into an index at sha1 (as one byte)
                // at literal value (so the hex value is the two chars)
    ...
    switch (flag)
    {
    case 'i': // milestone 2, print fs info
        print_fs_info();
        break;
    case 'l':
        list_root_dir();
